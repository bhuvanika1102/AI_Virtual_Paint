    ...
    def drawLandmarks(self,frame,data,auto_draw=False):
        if auto_draw:
            ...
            for myHand in allhands:
                ...
                for i in myHand:
                    cv2.circle(frame,(i[0],i[1]),4,(23,90,10),1)#landmark position,radius,colour,thickness
                for i in myHand:
...
def findError(knowngestures,unknownMatrix,keypoints):#keypoints=>indices of landmark used in distance matrix
    error=9999999#to ensure any actual error value will be smaller
    idx=-1#to store the index of known gesture
    for i in range(len(knowngestures)):
        currenterror=0
        for rows in keypoints:
            for columns in keypoints:
                currenterror+=abs(knowngestures[i][rows][columns]-unknownMatrix[rows][columns])#i-th known gesture
